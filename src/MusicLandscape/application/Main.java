...
public class Main {
    ...
    private class Menu {
        ...
        private MenuItem[] menu = {
                ...
                new MenuItem("reset") {
                    void execute() {
                        db.reset();
                        System.out.println("selection was reset");
                    }
                    // end of MenuItem
                },
                ...
                new MenuItem("show all artists"){
                    void execute(){
                        System.out.printf("show all artists:\n");

                        Main.this.showAllArtists();
                    }
                },

                new MenuItem("export to xml"){
                    void execute(){
                        System.out.printf("export to xml:\n");
    ...
    public void add() {
        ...
        try {
            ...
            if (inputYear >= 1900 && inputYear <= 2999) {
                ...
                System.out.println("Year was outside the valid range (1900 - 2999)");
            }
        } catch (NumberFormatException e) {
    ...
    public void exportXML(){
        try {
            Scanner sc = new Scanner(System.in);
            System.out.print("Enter file name (*.xml): ");
            String fileName = sc.nextLine();
            FileWriter file = new FileWriter(fileName);
            XMLFormatter xmlFormatter = new XMLFormatter();
            MyWriter myWriter = new MyWriter<>(file, xmlFormatter);
            int counter = 0;
            ...
            if (db.selection().length > 0) {
                for (Track t : db.selection()) {
                    if (myWriter.put(t)) {
                        ...
                        System.out.println("Could not write track: " + xmlFormatter.format(t));
                    }
                }
            }
